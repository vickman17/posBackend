...
if (
    ...
    isset($data->phoneNumber) && isset($data->password) && isset($data->role)
) {
    ...
    if ($checkStmt) {
        ...
        if ($checkStmt->num_rows > 0) {
            ...
            $sql = "INSERT INTO employees (username, hire_date, position, phoneNumber, password, employee_tag, role) VALUES (?, ?, ?, ?, ?, ?, ?)";
            $stmt = $conn->prepare($sql);

            if ($stmt) {
                $stmt->bind_param("sssssss", $username, $hire_date, $position, $phoneNumber, $hashedPassword, $employee_tag, $role);
